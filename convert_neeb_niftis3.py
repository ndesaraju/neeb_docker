...
def main(subject):
    ...
    for folder in folders:
        ...
        if contents and ("ima" in contents[0]): # "ima"
            ...
            if (files):
                rename(files)
                call(['/data/henry2/niharika/scripts/neeb_docker_utils/dcmgz2nii', '-o', folder, folder])
        else:
            for content in contents:
                files = glob.glob(content+'/*.ima') # "ima"
                if(files):
                    rename(files)
                    call(['/data/henry2/niharika/scripts/neeb_docker_utils/dcmgz2nii', '-o', content, content])
    return
...
def smooth_maps(subject):
    ...
    for mm in [0.5, 0.8, 1]:
        call(['/data/henry2/niharika/scripts/neeb_docker_utils/fslmaths', myelin, '-s', str(mm), subject+'/Myelin/MyelinMapsSmooth/MyelinMap_Smoothed_'+('%02d' % int(mm*10))+'.nii.gz'])
